$(function() {
	...
	function backOut() {
        ...
        $("#index_form").form("submit", {
            url : "/index_backupin.do",
            success : function(data) {
                data = $.parseJSON(data);
                if (data.success) {
                    $.messager.alert('温馨提示', data.msg, 'info' , function(){
                        $("#index_backup").dialog("close");
                    });
                } else {
    ...
    //备份
    function backupin() {
        $.messager.alert('温馨提示', "确定备份数据库吗?", 'info', function() {
            $.post("/index_backup.do", function(data) {
                if (data.success) {
                    $.messager.alert('温馨提示', data.msg, 'info');
                } else {
                    $.messager.alert('温馨提示', data.msg, 'info');
	...
	$("#index_backup").dialog({
		...
		height : 200,
		closed : true,
		buttons : "#index_btn_backup"
	});
    $("#backupin").bind("click",backupin);
    $("#restore").bind("click",backOut);
	$("#backupout").bind("click", function() {
		$("#index_backup").dialog("open");
		$("#index_form").form("clear");
