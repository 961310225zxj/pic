$(function () {
    ...
    function checkstatusFormatter(value, row, index) {
        ...
        if(value==3){
            return "<font color='#9acd32'>已上报</font>";
        }
        if(value>4){
    ...
    var methodObj = {
        search: function() {
            ...
            var msg=$("#msg").val();
            $.messager.confirm("确认提示","确认进行此操作吗?",function (r) {
                if(r){
            ...
            if(rowData.checkstatus==3){
                $.messager.alert("温馨提示","该条数据已上报,无法审核");
                return;
            }
